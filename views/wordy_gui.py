...
class WordyGui(tkn.Tk):
    ...
    def load_screen(self):
        ...
        prg_bar = ttk.Progressbar(frm_load, orient=tkn.HORIZONTAL, length=300)
        lbl_load = ttk.Label(
            frm_load,
            text="Loading",
            font=("Rachana", 12),
    ...
    def search_screen(self):
        ...
        btn_add = ttk.Button(frm_src, text="Add Word", command=search_to_add)

        ecas.enable_ctrl_a_select(ent_word)
        aph.add_place_holder(ent_word, "Enter Word")
        aph.add_place_holder(txt_means, "Meanings will be displayed here", True)
        mro.make_read_only(txt_means)

        ent_word.grid(row=0, column=0, columnspan=2, ipady=3, pady=pd.TOP, sticky="e")
    ...
    def add_screen(self, word=""):
        ...
        scr_means = ttk.Scrollbar(frm_means)
        txt_means = tkn.Text(
            frm_means, height=17, width=58, wrap=tkn.WORD, yscrollcommand=scr_means.set
        )
        scr_means.config(command=txt_means.yview)
        scr_means.pack(side=tkn.RIGHT, fill=tkn.Y)
        txt_means.pack(side="left")
        ...
        ent_sa.grid(row=0, column=1, columnspan=6, ipady=3, pady=pd.TOP, sticky="e")
